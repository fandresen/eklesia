...
function AddPastRoleForm({ postes, onSave, onCancel }: AddPastRoleFormProps) {
  ...
  const [typeEglise, setTypeEglise] = useState<"Église principale" | "Église annexe">("Église principale");
  ...
  const handleSave = () => {
    if (!idPoste || !eglise || !anneeDebut) {
      setError("Fenoina farafaharatsiny ny Andraikitra, Fiangonana, ary Taona nanombohana.");
      return;
    }
    setError("");
    onSave({
  ...
  return (
    <div className="bg-subtle-light p-4 rounded-lg mt-4 space-y-4 border border-primary/20">
      <h3 className="text-base font-semibold text-primary">Hanampy traikefa vaovao</h3>
      {/* Andraikitra (Poste) */}
      <div>
        <label className="text-sm font-medium text-text-light" htmlFor="id_poste">Andraikitra</label>
        <select
          id="id_poste"
          value={idPoste || ""}
          onChange={(e) => setIdPoste(parseInt(e.target.value))}
        ...
        >
          <option value="" disabled>Safidio ny andraikitra</option>
          {postes.map(p => (
            <option key={p.id_poste} value={p.id_poste}>{p.nom_poste_mg}</option>
          ))}
        </select>
      </div>
      ...
      <div>
        <label className="text-sm font-medium text-text-light" htmlFor="church-name">Anaran’ny fiangonana</label>
        <input
          className="mt-1 block w-full rounded-lg bg-white border border-subtle-light h-12 px-4 text-text-light focus:ring-primary/50 focus:border-primary"
          id="church-name"
      ...
      {/* Karazana Fiangonana */}
      <div>
        <label className="text-sm font-medium text-text-light">Karazana Fiangonana</label>
        <div className="grid grid-cols-2 gap-2 mt-1">
          <label className={`flex cursor-pointer ... ${typeEglise === 'Église principale' ? 'bg-primary/20 border-primary' : 'bg-white'}`} htmlFor="organized">
            <input className="sr-only" id="organized" name="church-type" type="radio" value="Église principale" checked={typeEglise === 'Église principale'} onChange={() => setTypeEglise('Église principale')} />
            Fiangonan-dehibe
          </label>
          <label className={`flex cursor-pointer ... ${typeEglise === 'Église annexe' ? 'bg-primary/20 border-primary' : 'bg-white'}`} htmlFor="group">
            <input className="sr-only" id="group" name="church-type" type="radio" value="Église annexe" checked={typeEglise === 'Église annexe'} onChange={() => setTypeEglise('Église annexe')} />
            Zanam-piangonana
          </label>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="text-sm font-medium text-text-light" htmlFor="start-year">Taona nanombohana</label>
          <input
            className="mt-1 block w-full rounded-lg bg-white border border-subtle-light h-12 px-4 text-text-light focus:ring-primary/50 focus:border-primary"
            id="start-year" placeholder="YYYY" type="number"
            value={anneeDebut || ""}
            onChange={(e) => setAnneeDebut(e.target.value ? parseInt(e.target.value) : undefined)}
          />
        </div>
        <div>
          <label className="text-sm font-medium text-text-light" htmlFor="end-year">Taona Namaranana</label>
          <input
            className="mt-1 block w-full rounded-lg bg-white border border-subtle-light h-12 px-4 text-text-light focus:ring-primary/50 focus:border-primary"
            id="end-year" placeholder="YYYY (tsy fenoina raha mbola mitohy)" type="number"
            value={anneeFin || ""}
            onChange={(e) => setAnneeFin(e.target.value ? parseInt(e.target.value) : undefined)}
          />
        </div>
      </div>
      ...
      {/* Fanamarihana */}
      <div>
        <label className="text-sm font-medium text-text-light" htmlFor="notes">Zava-bita / Traikefa manan-danja (Optional)</label>
        <textarea
          className="mt-1 block w-full rounded-lg bg-white border border-subtle-light p-4 text-text-light min-h-24 resize-none"
          id="notes" placeholder="Fanamarihana hafa..."
          value={realisations}
      ...
      <div className="flex gap-4">
        <button onClick={onCancel} className="flex-1 rounded-xl bg-subtle-light px-4 py-3 text-base font-bold text-primary shadow-sm">
          Hanafoana
        </button>
        <button onClick={handleSave} className="flex-1 rounded-xl bg-primary px-4 py-3 text-base font-bold text-white shadow-lg">
          Hitahiry
        </button>
      </div>
    </div>
...
export default function Step2({ modifyStep, setTitle, formData, updateData, errors }: propsT) {
  // --- États Locaux ---
  ...
  const getPosteNameById = (id: number) => {
    return postesList.find(p => p.id_poste === id)?.nom_poste_mg || `Poste ID: ${id}`;
  };
          ...
          <div className="max-h-60 overflow-y-auto space-y-2 pr-2">
            {filteredPostes.length > 0 ? (
              filteredPostes.map((poste) => {
                const isSelected = responsabilitesSouhaitees.some(r => r.id_poste === poste.id_poste);
                return (
                  <label
                    key={poste.id_poste}
                    className={`flex items-center p-3 rounded-lg cursor-pointer ${isSelected ? 'bg-primary/10 border-primary' : 'bg-transparent'}`}
                  ...
                  >
                    ...
                    />
                    <span className="ml-3 text-base text-text-light">{poste.nom_poste_mg}</span>
                  </label>
                );
              })
            ) : (
              <p className="text-text-light/60 text-center p-4">Tsy misy andraikitra mifanaraka</p>
            )}
          </div>
        ...
        <Card>
          ...
          <div className="space-y-3">
            ...
            {responsabilitesPassees.map((resp, index) => (
              ...
              >
                <div>
                  <p className="font-semibold text-text-light">{getPosteNameById(resp.id_poste)}</p>
                  <p className="text-sm text-text-light/80">{resp.eglise} ({resp.annee_debut} - {resp.annee_fin || 'ankehitriny'})</p>
                </div>
                <button
